...
def score(tcm:                pd.DataFrame,
          ...
          formula:            pd.DataFrame = None,
          formula_tcm_links:  pd.DataFrame = None,
          weights:            dict = None
          ) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    """
    计算复方、中药和化合物的 HerbiV Score
    Args:
        tcm:                要计算 HerbiV Score 的中药的信息，格式与 get.get_tcm 的返回值相同。
        tcm_chem_links:     tcm 和 chem 中的中药-成分（化合物）连接信息。
        chem:               要计算 HerbiV Score 的化合物的信息，格式与 get.get_chemicals 的返回值相同。
        chem_protein_links: chem和拟分析靶点（蛋白）的化合物（中药成分）-蛋白质（靶点）连接信息。
        formula:            要计算 HerbiV Score 的复方信息，格式与 get.get_formulas 的返回值相同。
        formula_tcm_links:  formula 和 tcm 中的复方-中药连接信息。
        weights:            各靶点（蛋白）的权重，各权重的和应为 1。若为 None，计算时将自动替换为相等的值。
    Returns:
        包含 HerbiV Score 的复方、中药和成分（化合物）的信息
        tcm_and_score:      中药信息及 HerbiV Score
        chem_and_score:     成分（化合物）信息及 HerbiV Score
        formula_and_score:  复方信息及 HerbiV Score
    Examples:
        >>> from herbiv import get
        >>> protein1 = get.get_proteins('Ensembl_ID', ['ENSP00000381588', 'ENSP00000252519'])
        >>> chem_protein_links1 = get.get_chem_protein_links('Ensembl_ID', protein1['Ensembl_ID'], 0)
        >>> chem1 = get.get_chemicals('HVCID', chem_protein_links1['HVCID'])
        >>> tcm_chem_links1 = get.get_tcm_chem_links('HVCID', chem1['HVCID'])
        >>> tcm1 = get.get_tcm('HVMID', tcm_chem_links1['HVMID'])
        >>> formula_tcm_links1 = get.get_formula_tcm_links('HVMID', tcm1['HVMID'])
        >>> formula1 = get.get_formula('HVPID', formula_tcm_links1['HVPID'])
        >>> tcm2, chem2, formula2 = score(tcm1, tcm_chem_links1, chem1, chem_protein_links1, formula1, formula_tcm_links1)
        >>> tcm2
    ...
    protein_ids = chem_protein_links['Ensembl_ID'].unique()

    # 计算与各蛋白对应的 HerbiV Score
    for protein_id in protein_ids:
        # 计算每一个化合物的 HerbiV Score
        chem_and_score[protein_id + ' HerbiV Score'] = chem.loc[:, 'HVCID'].apply(lambda x: 1 - (1 - np.array(
            [*chem_protein_links.loc[(chem_protein_links['HVCID'] == x) & (chem_protein_links['Ensembl_ID'] == protein_id)]['Combined_score']])
                                                                                                 ).prod()
        )

        # 计算各中药的 HerbiV Score
        tcm_and_score[protein_id + ' HerbiV Score'] = tcm.loc[:, 'HVMID'].apply(lambda x: 1 - (1 - np.array(
            [*chem_and_score.loc[chem_and_score['HVCID'].isin(
                tcm_chem_links.loc[tcm_chem_links['HVMID'] == x]['HVCID'])][protein_id + ' HerbiV Score']])
                                                                                            ).prod()
                                                                             )

        # 若传入了复方相关信息，则还需计算各复方的 HerbiV Score
        if formula is not None:
            formula_and_score[protein_id + ' HerbiV Score'] = formula.loc[:, 'HVPID'].apply(lambda x: 1 - (1 - np.array(
                [*tcm_and_score.loc[tcm_and_score['HVMID'].isin(
                    formula_tcm_links.loc[formula_tcm_links['HVPID'] == x]['HVMID'])][protein_id + ' HerbiV Score']])
                                                                                                        ).prod()
                                                                                         )
    ...
    # 若使用默认权重，则权重默认均为1
    if weights is None:
        weights = {col: 1 for col in [protein_id + ' HerbiV Score' for protein_id in protein_ids]}

    # TODO: 将所有 Importance Score 替换为 HerbiV Score
